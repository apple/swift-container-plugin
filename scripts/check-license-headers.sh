...
@@ This source file is part of the SwiftContainerPlugin open source project
@@
@@ Copyright (c) YEARS Apple Inc. and the SwiftContainerPlugin project authors
@@ Licensed under Apache License v2.0
@@
@@ See LICENSE.txt for license information
@@ See CONTRIBUTORS.txt for the list of SwiftContainerPlugin project authors
...
read -ra PATHS_TO_CHECK_FOR_LICENSE <<< "$( \
  ...
  ":(exclude)Sources/ContainerRegistry/openapi/*" \
  ":(exclude)Tests/ContainerRegistryTests/Resources/*" \
  ":(exclude)Vendor/*" \
  ":(exclude)docker/*" \
  ":(exclude)**/*.docc/*" \
  ":(exclude)**/.gitignore" \
...
for FILE_PATH in "${PATHS_TO_CHECK_FOR_LICENSE[@]}"; do
  ...
  NORMALIZED_FILE_HEADER=$(
    echo "${FILE_HEADER}" \
    | sed -e 's/202[4]-202[4]/YEARS/' -e 's/202[4]/YEARS/' \
  )
